...
import { X, Camera as CameraIcon, Shield } from "lucide-react";
import { useToast } from "@/hooks/use-toast";

const ToxicityIdentifier = () => {
  const navigate = useNavigate();
  const { toast } = useToast();
  const videoRef = useRef<HTMLVideoElement>(null);
  const [stream, setStream] = useState<MediaStream | null>(null);
  const [isCapturing, setIsCapturing] = useState(false);
  const mode: 'plant' = 'plant'; // Use plant mode but will check toxicity in results
  ...
  const capturePhoto = async () => {
    ...
    try {
      ...
      // Navigate to scanning screen with image and toxicity flag
    ...
    <div className="fixed inset-0 bg-black z-50 pb-20">
      ...
      <div className="relative w-full h-full">
        ...
        <div className="absolute top-16 left-4 right-4 flex justify-center">
          <div className="bg-green-600/80 backdrop-blur-sm px-4 py-2 rounded-full">
            <div className="flex items-center gap-2 text-white">
              <Shield className="w-5 h-5" />
              <span className="font-semibold">Toxicity Identifier</span>
            </div>
          </div>
        </div>
...
export default ToxicityIdentifier;
